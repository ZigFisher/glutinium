#!/bin/sh
# Usage: ./hisi-load.sh [ -r|-i|-a ] [ sensor ]
#         -r : rmmod all modules
...
{
    # low power control
    # source ./hisi-lowpower.sh > /dev/null

    # pinmux configuration
    source ./hisi-pinmux.sh mii i2c > /dev/null

    # clock configuration
    source ./hisi-clkcfg.sh > /dev/null
    ...
    source ./hisi-sysctl.sh > /dev/null
}
...
load_usage()
{
    echo "Usage:  ./hisi-load.sh [-option] [sensor_name]"
    echo "options:"
    ...
    echo -e "for example: ./hisi-load.sh -a ar0130 \n"
}
