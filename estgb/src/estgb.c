...
void sendPicture(char *filename) {
	if (!access(filename, R_OK)) {
		estgbconf.ret = telebot_send_photo(estgbconf.handle, estgbconf.user_id,
				filename, true, estgbconf.comment, false, 0, NULL);
		if (estgbconf.ret != TELEBOT_ERROR_NONE) {
...
void sendVideo(char *filename) {
	if (!access(filename, R_OK)) {
		if (estgbconf.isAnimation)
			estgbconf.ret = telebot_send_animation(estgbconf.handle,
					estgbconf.user_id, filename, true, 0, 0, 0,
					estgbconf.comment, false, 0, NULL);
		else
			estgbconf.ret = telebot_send_video(estgbconf.handle,
					estgbconf.user_id, filename, true, 0, 0, 0,
					estgbconf.comment, false, 0, NULL);
		if (estgbconf.ret != TELEBOT_ERROR_NONE) {
...
void sendAudio(char *filename) {
	if (!access(filename, R_OK)) {
		estgbconf.ret = telebot_send_audio(estgbconf.handle, estgbconf.user_id,
				filename, true, 0, NULL, estgbconf.comment, false, 0, NULL);
		if (estgbconf.ret != TELEBOT_ERROR_NONE) {
...
void sendDocument(char *filename) {
	if (!access(filename, R_OK)) {
		estgbconf.ret = telebot_send_document(estgbconf.handle,
				estgbconf.user_id, filename, true, false, 0, NULL);
		if (estgbconf.ret != TELEBOT_ERROR_NONE) {
