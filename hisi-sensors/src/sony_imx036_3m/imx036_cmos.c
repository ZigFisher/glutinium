...
{
	cmos_inttime.full_lines_std = 1608;
	cmos_inttime.full_lines_std_30fps = 1608;
	cmos_inttime.full_lines = 1608;
	cmos_inttime.full_lines_limit = 65535;
	cmos_inttime.max_lines_target = 1606;
	cmos_inttime.min_lines_target = 2;
	cmos_inttime.vblanking_lines = 1608;

	cmos_inttime.exposure_ashort = 0;

	cmos_inttime.lines_per_500ms = 16874;
	cmos_inttime.flicker_freq = 0;
...
static __inline void cmos_inttime_update(cmos_inttime_ptr_t p_inttime) 
{
    ...
    //printf("exp = %d, ", p_inttime->exposure_ashort);	
}
		...
		HI_U32 exposure_shift)
{
	...
	p_gains->again = (exposure << p_gains->again_shift) / exposure0; 
       ...
       //printf("exposure = %d, exposure_max = %d, exposure0 = %d, ", exposure, exposure_max, exposure0);
