...
static cmos_isp_default_t st_coms_isp_default_lin = 
{
	...
	// gain
	0x8,	0x04, // this is gain target, it will be constricted by sensor-gain.
...
static cmos_isp_default_t st_coms_isp_default_wdr = 
{
	...
	{	0x0200, 0x80a9, 0x805b,
		0x8065, 0x0199, 0x8038,
		0x8015, 0x80d0, 0x01e2
	},
...
static /*__inline*/ cmos_inttime_const_ptr_t cmos_inttime_initialize()
{
	...
	cmos_inttime.full_lines_std = 1122;
#else
	cmos_inttime.full_lines_std = 1573;
#endif	
	cmos_inttime.full_lines_std_30fps = 1573;
	cmos_inttime.full_lines = 1573;
	cmos_inttime.full_lines_del = 1573; //TODO: remove
	cmos_inttime.full_lines_limit = 65535;
	//HDR mode,maximum coarse_integration_time = minimum(70 x T1 / T2, frame_length_lines - 71)
	cmos_inttime.max_lines = 1120;
	...
	cmos_inttime.lines_per_500ms = 1573*30/2; 
...
static __inline void cmos_gains_update(cmos_gains_const_ptr_t p_gains)
{
	...
	switch(ag)
	{
		case(0):
			...
			//sensor_write_register(0x3198, 0x061E);  //motion_detect_q1
			break;
		case(1):
			sensor_write_register(0x3060, 0x1300);
			//sensor_write_register(0x3198, 0x061E);
			break;
		case(2):
			sensor_write_register(0x3060, 0x1310);
			//sensor_write_register(0x3198, 0x062A);
			break;
		case(4):
			sensor_write_register(0x3060, 0x1320);
			//sensor_write_register(0x3198, 0x063C);
			break;
		case(8):
			sensor_write_register(0x3060, 0x1330);
			//sensor_write_register(0x3198, 0x0655);
			break;
	}
...
void setup_sensor(int isp_mode)
{
	...
	{
#if defined(APTAR0331_LIN_CMOS)	
		sensor_write_register(0x300A, 0x0625);
		sensor_write_register(0x300C, 0x0625);
#elif defined(APTAR0331_WDR_CMOS)
		sensor_write_register(0x300A, 0x08A1);
		sensor_write_register(0x300C, 0x0460);
#endif
	}
	else if(1 == isp_mode) /* ISP pixel calibration isp_mode */
	{
		sensor_write_register(0x300C, 0x3390);
