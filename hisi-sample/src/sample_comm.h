...
typedef struct hithreadArgs_tag {
    SOC_TYPE socType;
...
HI_S32 SAMPLE_COMM_ISP_Init(WDR_MODE_E  enWDRMode, SAMPLE_VI_MODE_E sensorType);
HI_VOID SAMPLE_COMM_ISP_Stop(void);
HI_S32 SAMPLE_COMM_ISP_Run(void);
HI_S32 SAMPLE_COMM_ISP_ChangeSensorMode(HI_U8 u8Mode);

HI_S32 SAMPLE_COMM_VI_GetSizeBySensor(PIC_SIZE_E *penSize, SAMPLE_VI_MODE_E enMode);
HI_S32 SAMPLE_COMM_VI_GetRgbirAttrBySensor(ISP_RGBIR_ATTR_S *pstRgbirAttr, SAMPLE_VI_MODE_E enMode);
HI_S32 SAMPLE_COMM_VI_GetRgbirCtrlBySensor(ISP_RGBIR_CTRL_S *pstRgbirCtrl, SAMPLE_VI_MODE_E enMode);
...
HI_S32 LoadSensorLibrary(const char *libsns_path);
