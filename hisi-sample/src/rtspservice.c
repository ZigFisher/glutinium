...
*/
	...
	if((pStr = strstr(s8TranStr, RTSP_RTP_AVP)))//pStr="RTP/AVP;unicast;client_port=5004-5005"
	{
		//Transport: RTP/AVP
		...
		{
			//单播
			if (strstr(s8TranStr, "unicast"))//pStr="unicast;client_port=5004-5005"
			{
				//如果指定了客户端端口号，填充对应的两个端口号
				if( (pStr = strstr(s8TranStr, "client_port")) )//pStr="client_port=5004-5005"
				{
...
void RTSP_state_machine(RTSP_buffer * pRtspBuf, int method)
{
	...
	*/
	...
	/*打开会话列表*/
	pRtspSess = pRtspBuf->session_list;//如果有连接就会在addclient里里初始化session_list，所以有连接session_list就不会为NULL
	if (pRtspSess == NULL)
	{
		return;
	}
...
**返回值:
	ERR_NOERROR:			有很多原因，分情况:无内存，正常等都产生这个错误
	ERR_GENERIC:			内部错误
	ERR_CONNECTION_CLOSE:	连接关闭
**
**************************************************************************************************/
...
**
**************************************************************************************************/
//base64_encode2(ringfifo[iput].buffer+off,9,psp.base64sps, 512);
//base64_encode2(ringfifo[iput].buffer+off, 4, psp.base64pps, 512);  00 00 00 01 68 ce 3c 80
...
void UpdateSps(unsigned char *data,int len)
{
	...
	//base64_encode2(ringfifo[iput].buffer+off,9,psp.base64pps, 512);
...
void UpdatePps(unsigned char *data,int len)
{
	...
	//printf("\nin pps");
	//base64_encode2(ringfifo[iput].buffer+off, 4, psp.base64pps, 512);
