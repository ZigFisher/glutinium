...
//把RTP会话添加进schedule中，并设置播放动作为发送，错误返回-1,正常返回schedule队列号
//参数:in,RTP会话
...
/*
发送响应,将错误代码err与rtsp中CSEQ写到缓冲区去
err:错误代码
addon:是否添加字符串，如果内容太多，这个参数为增加的内容
rtsp:主要是利用里面的CSEQ
*/
