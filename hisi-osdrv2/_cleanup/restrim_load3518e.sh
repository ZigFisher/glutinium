...
os_mem_size=`cat /proc/cmdline | sed -e 's/^.*mem=//' -e 's/[ M].*$//'`;
mmz_start=0x82500000;       # mmz start addr
mmz_size=27M;               # 27M, mmz size

echo "OS mem size = $os_mem_size MB"
...
insert_sns()
{
	case $SNS_TYPE in
		...
		ov9732)
			...
			if [ "$hw_type" != "404" ] && [ "$hw_type" != "421" ]; then
				himm 0x200f0084 0x1				# VI_DATA12
			fi
		...
		ov2735)
			...
			himm 0x200f0000 0x00000001;
			himm 0x200f0004 0x00000000;
			;;
...
sys_config()
{
	...
	eth_val=`perictrl --get-hw-prop=eth`
	if [ "$eth_val" = "0" ]; then
		echo "NOT Setting PINMUX for NET!"
	else
		echo "Setting PINMUX for NET!"
...
#	insmod hi3518e_ive.ko	
    ...
    insmod hi_cipher.ko
    insmod hi_rtc.ko
    insmod wdt.ko nowayout=1 nodeamon=1
}
